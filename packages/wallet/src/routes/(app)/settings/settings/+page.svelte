<script lang="ts">
  import { getYakklAccounts, setYakklAccountsStorage } from '$lib/common/stores';
  ...
  import Back from '$lib/components/Back.svelte';
	import type { BigNumberish, YakklAccount } from '$lib/common';
  ...
  async function handleClick(idx: number) {
    try {
      if (accounts) {
        ...
        $form.alias = alias = accounts[index].alias;
  ...
  async function handleDelete() {
    try {
      if (index >= 0) {
        let _deletedValue = accounts.splice(index,1);
...
<div class="text-center min-h-[75rem]">
  <div class="relative w-full h-9">
    ...
    <h2 class="text-xl tracking-tight font-extrabold text-base-content">
      <span class="lg:inline">System Settings</span>
    </h2>
  </div>
  ...
  <div class="justify-center mb-4 mt-2">
    <Button>Account List<Icon.ChevronDoubleUpOutline name="chevron-down-solid" class="w-3 h-3 ml-2 text-white dark:text-white" /></Button>
  ...
  <form class="" on:submit|preventDefault={handleSubmit}>
    ...
    <div class="flex flex-row mt-2">
      <div class="flex-col w-full text-left">
        <span class="mt-2 text-left text-xs text-base-content font-bold mb-1">Account Name:</span>
        <input
          id="name"
        ...
        <span class="mt-2 text-left text-xs text-base-content font-bold mb-1">Account Type:</span>
        <input
          id="accountType"
          class="input input-bordered input-primary w-full mb-2"
