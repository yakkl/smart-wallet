...
<Modal bind:show title={account ? 'Edit Account' : 'Add Account'}>
  <form on:submit|preventDefault={handleSubmit} class="space-y-4 p-6">
    <div>
      ...
      <div class="mt-1 flex items-center">
        ...
        <button
          ...
          on:click={() => copyAddress(account?.address || '')}
        >
    ...
    <div>
      ...
      <textarea id="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm text-gray-800" bind:value={$form.description} on:change={handleChange}></textarea>
