...
class Logger {
  ...
  private backend: string = "console"; // Default logging backend
...
//    */
  ...
  private log(
    ...
    switch (this.backend) {
      case "localStorage":
        const logs = JSON.parse(localStorage.getItem("yakklLogs") || "[]");
        logs.push({ timestamp, label, message });
        localStorage.setItem("yakklLogs", JSON.stringify(logs));
        break;

      case "indexedDB":
        const dbPromise = openDB("YakklLogsDB", 1, {
          upgrade(db) {
        ...
        dbPromise.then(async (db) => {
          ...
          await store.add({ timestamp, label, message });
        });
        break;

      case "dexie":
        const dexieDB = new Dexie("YakklDexieLogs");
        dexieDB.version(1).stores({ logs: "++id, message, timestamp" });
        dexieDB.table("logs").add({ timestamp, label, message });
        break;

      // case "sqlite":
      //   saveToSQLite({ timestamp, label, message });
      //   break;

      // case "background":
      //   browser_ext.runtime.sendMessage({ type: "LOG_MESSAGE", log: { timestamp, label, message } });
      //   break;
...
//     // ADD other references to LoggerDixe, LoggerBackground, Logger<whatever> so that they all use this and only process what is needed for storing or sending for their given environment
//   }
